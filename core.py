...
class ClusterAllocator:
    ...
    def __init__(self):
        ...
        self._pool_pci = {
            'ERICSSON': {b: list(range(504)) for b in ["700","800","900","1800","2100","2600","1","3500","78"]},
            'HUAWEI':   {b: list(range(504)) for b in ["700","800","900","1800","2100","2600","1","3500","78"]}
        }
        self._pool_rsi = {
            'ERICSSON': {b: list(range(838)) for b in ["700","800","900","1800","2100","2600","1","3500","78"]},
            'HUAWEI':   {b: list(range(838)) for b in ["700","800","900","1800","2100","2600","1","3500","78"]}
        }
    ...
    def get_unused_pci(self, vendor: str, band: str, used_set: set, min_pci: int=0) -> list:
        """Devuelve pool – usado_maestro – usado_por_clusters, filtrado por min_pci."""
        pool = self._pool_pci.get(vendor.upper(), {}).get(band, [])
        assigned_global = set().union(*self._assigned_by_cluster.values()) if self._assigned_by_cluster else set()
        forbidden = set(used_set) | assigned_global
        return [p for p in pool if p not in forbidden and p >= min_pci]

    def get_unused_rsi(self, vendor: str, band: str, used_set: set, min_rsi: int=0) -> list:
    ...
    def register_assigned(self, cluster: set, pcis: list):
        ...
        self._assigned_by_cluster.setdefault(key, set()).update([p for p in pcis if isinstance(p, int)])
...
def map_column_names(df: pd.DataFrame) -> pd.DataFrame:
    mapping = {
        ...
        'RSQID':       ['RSQID', 'RSI', 'RSRQID', 'ROOTSEQUENCEINDEX', 'LOGICALROOTSEQUENCEINDEX'],
        'TAC':         ['TAC'],
...
def detectar_numero_sectores(site: str, df_pci_master: pd.DataFrame) -> int:
    ...
    if df_site_local.empty:
        return 3
    sufijos = {int(m.group(1)) for cel in df_site_local['CELLNAME'].dropna().astype(str)
               for m in [re.search(r"(\d+)[AB]$", cel.strip().upper())] if m}
    return len(sufijos) if sufijos else 3


def generar_nombre_celda(node: str, tech: str, band: str, idx: int, suffix: str = "A") -> str:
...
def cargar_y_preprocesar_pci(csv_pci_path: str) -> pd.DataFrame:
    ...
    df = pd.read_csv(csv_pci_path, dtype=str, sep=sep, encoding='utf-8', error_bad_lines=False)
    ...
    df['BAND_CLEAN'] = df.apply(lambda r: normaliza_banda(r.get('BAND',''), r.get('TECH','')), axis=1)
    df['TECH_GROUP'] = df.get('TECH','').apply(agrupar_tech)
    df['VENDOR_CLEAN'] = df.get('VENDOR','').astype(str).str.strip().str.upper()
    return df


def cargar_y_preprocesar_rsi_5g(csv_rsi_path: str, df_pci_master: pd.DataFrame) -> pd.DataFrame:
    sep = detect_separator(csv_rsi_path)
    try:
        df = pd.read_csv(csv_rsi_path, dtype=str, sep=sep, encoding='utf-8', error_bad_lines=False)
    except FileNotFoundError:
        return pd.DataFrame()
    df = map_column_names(df)
    ...
    df['TAC'] = df['SITE_CLEAN'].map(
        df_pci_master.set_index('SITE_CLEAN')['TAC']
    )
    return df
...
def sugerir_pci_rsi(
    ...
    tacs = [t for t in tacs if df_site[(df_site['TAC']==t)&(df_site['TECH_GROUP']=='NBIOT')].empty]
    ...
    for tac_item in tacs:
        ...
        libres_pci = allocator.get_unused_pci(df_site['VENDOR_CLEAN'].iloc[0], bc, usados_pci, min_pci)

        usados_rsi = serie_a_enteros_multi(
            df_pci_master[mask_pci]['RSQID']
        ) if tc != '5G' else set()
        libres_rsi = allocator.get_unused_rsi(df_site['VENDOR_CLEAN'].iloc[0], bc, usados_rsi, min_rsi)
        ...
        ar_list = sugerir_rsi_con_sep(libres_rsi, n_celdas, df_site['VENDOR_CLEAN'].iloc[0], bc)
...
def planificar_lnr700(
    site: str, n_celdas: int,
    df_pci_master: pd.DataFrame,
    df_rsi_5g_master: pd.DataFrame,
    tac_a_vecinos: dict,
    min_pci: int, min_rsi: int,
    modo_r: bool, manual_cache: dict
) -> tuple[list, list, list, list]:
    allocator = ClusterAllocator()
    res4, det4 = sugerir_pci_rsi(site, site, "4G", "700", n_celdas,
                                   df_pci_master, df_rsi_5g_master,
                                   tac_a_vecinos, min_pci, min_rsi,
                                   modo_r, allocator)
    coord = [d.get("PCI sugerido") % 3 if isinstance(d.get("PCI sugerido"), int) else None
             for d in det4]
    res5, det5 = sugerir_pci_rsi(site, site, "5G", "700", n_celdas,
                                   df_pci_master, df_rsi_5g_master,
                                   tac_a_vecinos, min_pci, min_rsi,
                                   modo_r, allocator, coord)
    ...
    for (site, band), group in df_req.groupby(['SITE', 'BAND_CLEAN']):
        ...
        if band == "700":
            r4, d4, r5, d5 = planificar_lnr700(
                site, n_celdas, df_pci_master, df_rsi_5g_master,
                tac_a_vecinos, 0, 0, modo_r, {}
            )
            ...
            r, d = sugerir_pci_rsi(
                site, site, tech, band, n_celdas,
                df_pci_master, df_rsi_5g_master,
                tac_a_vecinos, 0, 0, modo_r
            )
            resumen_all.extend(r)
            detalle_all.extend(d)
    pd.DataFrame(resumen_all).to_csv(salida_resumen, index=False, sep=';', encoding='utf-8-sig')
    pd.DataFrame(detalle_all).to_csv(salida_detalle, index=False, sep=';', encoding='utf-8-sig')
    print("Resumen masivo generado:")
    print(tabulate.tabulate(pd.DataFrame(resumen_all), headers='keys', tablefmt='github', showindex=False))
    print("\nDetalle masivo generado:")
    print(tabulate.tabulate(pd.DataFrame(detalle_all), headers='keys', tablefmt='github', showindex=False))
