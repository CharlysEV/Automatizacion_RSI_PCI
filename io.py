...
def parse_args():
    ...
    parser.add_argument(
        "-i", "--entrada",
        help="CSV de entrada (modo masivo) o SITE (modo individual)"
    )
    parser.add_argument(
        "-z", "--zr-corr",
        default="",
        help="CSV de correspondencias ZR (opcional)"
    )
    parser.add_argument(
        "-t", "--tech",
    ...
    parser.add_argument(
        "-b", "--band",
        required=True,
        help="Banda (700,800,1800,2100,2600,3500,78,1)"
    )
    parser.add_argument(
        "--min-pci", type=int, default=0,
        help="Valor mínimo de PCI"
    )
    parser.add_argument(
        "--min-rsi", type=int, default=0,
        help="Valor mínimo de RSI"
    )
    parser.add_argument(
        "-o", "--output-dir",
        default="salida",
...
def main():
    ...
    df_rsi_5g = cargar_y_preprocesar_rsi_5g('gnodebfunctionmodule_nrducell.csv', df_pci_master)
    ...
    if args.masivo:
        ...
        resumen_csv = os.path.join(
            args.output_dir,
            f"resumen_masivo_{datetime.now().strftime('%Y%m%d')}.csv"
        )
        detalle_csv = os.path.join(
            args.output_dir,
            f"detalle_masivo_{datetime.now().strftime('%Y%m%d')}.csv"
        )
